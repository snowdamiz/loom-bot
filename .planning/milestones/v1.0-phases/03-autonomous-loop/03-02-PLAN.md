...
must_haves:
  truths:
    ...
    - "AgentLoop accepts optional evaluator and replanner, calling evaluateOutcome after each sub-goal and triggering replan on divergence"
  artifacts:
  ...
  key_links:
    ...
    - from: "apps/agent/src/loop/agent-loop.ts"
      to: "apps/agent/src/loop/evaluator.ts"
      via: "Optional evaluator called after each sub-goal in runGoalCycle"
      pattern: "this\\.evaluator\\.evaluateOutcome"
    - from: "apps/agent/src/loop/agent-loop.ts"
      to: "apps/agent/src/loop/replanner.ts"
      via: "Optional replanner triggered when evaluator detects divergence"
      pattern: "this\\.replanner\\.replan"
---
...
- Export class `AgentLoop` with constructor taking:
  ...
  - `evaluator?: Evaluator` (optional — wired by Supervisor in Plan 05; import type from `./evaluator.js`)
  - `replanner?: Replanner` (optional — wired by Supervisor in Plan 05; import type from `./replanner.js`)
  - `config?: { maxTurnsPerSubGoal?: number; cycleSleepMs?: number }`
  - Store evaluator and replanner as instance fields. They are optional at construction time because Plan 02 runs before Plan 05 creates the concrete classes, but Supervisor (Plan 05) will always provide them when creating AgentLoop instances.
...
- Method `runGoalCycle(goalId: number): Promise<void>`:
  ...
  - Maintain an evaluations array: `Array<{ divergent: boolean; severity: string }>` (reset per cycle)
  ...
  - Check cancellation flag at top of each iteration — if `this.cancelled`, break immediately
  - Execute sub-goal via executeSubGoal()
  - **LOOP-03 evaluation (if evaluator is wired):**
    - Call `this.evaluator.evaluateOutcome(subGoal, result.outcome, goalDescription)` after executeSubGoal()
    - Push evaluation result onto evaluations array
    - Call `this.evaluator.shouldReplan(goalId, evaluations)` — if true AND this.replanner is wired:
      - Call `this.replanner.replan(goalId, evaluation.reason ?? 'divergence detected')`
      - If replan result is `{ escalated: true }`: break the loop (goal paused for operator)
      - If replan result is `{ replanned: true }`: continue loop (sub-goals have been re-decomposed, getNextSubGoal will pick up new ones)
  - Log cycle complete
