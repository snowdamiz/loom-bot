...
must_haves:
  truths:
    ...
    - "AgentLoop has a cancel() method that sets a flag checked at each loop iteration, enabling graceful shutdown by Supervisor"
  artifacts:
...
<task type="auto">
  <name>Task 2: Create supervisor and result collector, add cancel() to AgentLoop</name>
  <files>
    apps/agent/src/loop/agent-loop.ts
    apps/agent/src/multi-agent/supervisor.ts
    apps/agent/src/multi-agent/result-collector.ts
...
- `evaluator: Evaluator` (from `../loop/evaluator.js`)
- `replanner: Replanner` (from `../loop/replanner.js`)
...
- Create new AgentLoop instance for this goal, passing `this.evaluator` and `this.replanner` to the constructor (so runGoalCycle can call evaluateOutcome and replan)
- Store in activeLoops map
- Start `agentLoop.runGoalCycle(goalId)` in background (fire-and-forget with error handler)
- Log spawn to stderr

**Update `apps/agent/src/loop/agent-loop.ts`** — add cancel() method:
- Add a `private cancelled = false` field to AgentLoop class
- Add method `cancel(): void` — sets `this.cancelled = true`
- `runGoalCycle()` already checks `this.cancelled` at the top of each loop iteration (added in Plan 02 revision) and breaks if true
- `runContinuousLoop()` should also check `this.cancelled` at the top of each iteration
- `executeSubGoal()` should check `this.cancelled` at the top of each tool-calling turn and return `{ success: false, outcome: 'cancelled' }` if true

Method `stopMainAgent(goalId: number): Promise<void>`:
- Call `agentLoop.cancel()` to set the cancellation flag
- Remove from activeLoops
- The AgentLoop will break out of its current loop iteration on the next check
