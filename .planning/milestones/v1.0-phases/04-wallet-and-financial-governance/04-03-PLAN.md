...
must_haves:
  truths:
    ...
    - "Agent is notified of inbound SOL transfers (via accountNotifications) AND SPL token transfers (via programSubscribe on TOKEN_PROGRAM_ID and TOKEN_2022_PROGRAM_ID) through WebSocket subscriptions"
  ...
  artifacts:
    ...
    - path: "packages/wallet/src/client/subscribe.ts"
      provides: "WebSocket subscriptions for inbound SOL and SPL token monitoring (TOKEN_PROGRAM_ID + TOKEN_2022_PROGRAM_ID)"
...
<task type="auto">
  ...
  <action>
    ...
    **Inbound monitoring (subscribe.ts):**
    ...
    Types:
    - `InboundEvent`: discriminated union on `type` field:
      - `{ type: 'sol'; lamports: string }` — SOL balance change detected
      - `{ type: 'token'; mint: string; amount: string; decimals: number }` — SPL token balance change detected

    - `subscribeToWallet(db, onInbound: (event: InboundEvent) => void): Promise<{ stop: () => void }>`
      ...
      - **SOL inbound:** Subscribes to `accountNotifications` for the wallet address (native SOL balance changes). On notification: calls `onInbound({ type: 'sol', lamports: newBalance })`.
      - **SPL token inbound:** Subscribes to token account changes using `programSubscribe` (or `onProgramAccountChange` equivalent) for BOTH `TOKEN_PROGRAM_ID` and `TOKEN_2022_PROGRAM_ID`, filtered to accounts owned by the wallet's public key. Use memcmp filter on the owner field (bytes 32-64 of token account data) to scope notifications to the wallet's associated token accounts. On notification: parse the token account data to extract mint, amount, and decimals, then call `onInbound({ type: 'token', mint, amount, decimals })`.
        - NOTE: @solana/kit's subscriptions API may not support programSubscribe directly. If so, use @solana/web3.js v1 `Connection.onProgramAccountChange(TOKEN_PROGRAM_ID, callback, 'confirmed', [{ memcmp: { offset: 32, bytes: walletPublicKey } }])` and the same for TOKEN_2022_PROGRAM_ID. This creates two additional subscriptions alongside the SOL subscription.
      - Returns an object with a `stop()` method that aborts/unsubscribes all three subscriptions (SOL + TOKEN_PROGRAM_ID + TOKEN_2022_PROGRAM_ID)
      - Implements reconnection: if any subscription closes unexpectedly, wait 5 seconds and reconnect all. Log reconnection attempts to stderr. Maximum 10 consecutive reconnection attempts before giving up.
    ...
    - Export InboundEvent type from subscribe.ts.

    **Update index.ts:** Re-export sendSol, sendSplToken, SendResult, getTransactionHistory, subscribeToWallet, InboundEvent.
  ...
  <verify>
    ...
    - subscribe.ts subscribes to SOL (accountNotifications) AND SPL tokens (programSubscribe for both TOKEN_PROGRAM_ID and TOKEN_2022_PROGRAM_ID)
  ...
  <action>
    ...
    **Agent process wiring (apps/agent/src/index.ts):**
    ...
    1. **Start signer co-process:**
       ...
       - Wait for signer to be ready: the signer sends a "ready" message via IPC (process.send, added in Plan 01 Task 2). The agent waits for this message with a 10-second timeout.
...
- Agent can receive SOL and SPL tokens and is notified via WebSocket subscriptions (accountNotifications for SOL, programSubscribe for TOKEN_PROGRAM_ID and TOKEN_2022_PROGRAM_ID)
