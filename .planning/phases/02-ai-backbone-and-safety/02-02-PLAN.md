...
Output: `apps/cli` binary with kill/resume commands, `invokeWithKillCheck` wrapper in @jarvis/tools, agent process wired with KillSwitchGuard and ModelRouter.
</objective>
...
<task type="auto">
  <name>Task 2: Add TOOL-06 kill switch gate and wire agent startup</name>
  <files>
    packages/ai/src/kill-switch.ts
    packages/tools/src/invoke-safe.ts
    packages/tools/src/index.ts
  ...
  <action>
    ...
    **Part C: Wire agent startup (apps/agent/src/index.ts)**
  ...
  <verify>
    ...
    4. Test kill switch blocking: First activate kill switch via CLI (Task 1), then verify that attempting to call invokeWithKillCheck would throw. Can verify by running a small tsx script:
       ```
       DATABASE_URL=... tsx -e "
    ...
    5. Then resume via CLI and re-run -- should print "NOT BLOCKED"
  </verify>
  <done>
    invokeWithKillCheck gates every tool call with kill switch check (TOOL-06). activateKillSwitch/deactivateKillSwitch helper functions exist in @jarvis/ai. Agent process creates KillSwitchGuard and ModelRouter at startup. Kill switch enforcement path: CLI -> DB flag -> KillSwitchGuard cache -> assertActive() throw -> blocks AI calls (router) and tool calls (invokeWithKillCheck).
  </done>
</task>
...
- `jarvis resume "test"` deactivates kill switch and creates audit row
