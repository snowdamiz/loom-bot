...
must_haves:
  truths:
    ...
    - "Agent can query its own P&L to inform planning decisions"
    - "AI inference costs are auto-inserted into operating_costs when logged to ai_calls"
  artifacts:
  ...
  <action>
    ...
    **packages/db/src/schema/pnl-view.ts** (COST-04, COST-05):
    ...
    - Export async function `getAiSpendSummary(db: DbClient): Promise<{ totalCostUsd: number; totalCalls: number; byTier: Record<string, { calls: number; costUsd: number }> }>`:
      Aggregate ai_calls table: total cost, total calls, grouped by tier. This is the agent's view into its own AI spending (COST-05).
...
- AI spend breakdown available by tier via getAiSpendSummary
</success_criteria>
