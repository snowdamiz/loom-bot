  ...
  <action>
    ...
    1. **`packages/tools/src/types.ts`** — Core type definitions:
       ...
       export interface ToolDefinition<TInput = unknown, TOutput = unknown> {
         ...
         maxOutputBytes?: number;     // output size cap for ToolResult ONLY (protects LLM context). Full output is ALWAYS logged to Postgres.
    ...
    4. **`packages/tools/src/invoke.ts`** — Logging-wrapped invocation:
       ...
       **`invokeWithLogging(registry, db, toolName, rawInput, overrideTimeoutMs?)`**:
       ...
       - On success: call `logToolComplete(db, { parentId, output: rawOutput, durationMs })` with the FULL raw output (per locked decision: "Log everything: complete shell output. Storage is cheap, missing data is not recoverable"). Then, AFTER logging, apply output size limit for the ToolResult returned to the caller: if `maxOutputBytes` is set and `JSON.stringify(rawOutput).length > maxOutputBytes`, create a truncated copy and set `truncated: true` in the ToolResult. The logged output in `tool_calls.output` is ALWAYS the complete, untruncated data. Only the ToolResult returned to the caller (which protects LLM context from huge payloads) gets truncated.
       ...
       **Critical: Truncation is a caller-facing concern, NOT a logging concern.** The invocation layer has two distinct outputs: (1) the log row in Postgres (always full data), and (2) the ToolResult returned to the caller (may be truncated to protect LLM context window). These MUST be handled separately.
  </action>
  <verify>
    ...
    - Verify full output logging: create a mock tool that returns a large payload (>maxOutputBytes), invoke it — confirm tool_calls.output in Postgres contains the FULL untruncated output, while the returned ToolResult.output is truncated with `truncated: true`
  </verify>
  <done>
    ToolRegistry supports register/unregister/invoke, ToolDefinition interface includes zod schema and AbortSignal, timeout wrapper enforces configurable limits (TOOL-07), and invokeWithLogging guarantees pre-execution logging (TOOL-05) + post-execution logging with FULL output (LOG-02) for every invocation. Truncation is applied ONLY to the ToolResult returned to the caller (protecting LLM context), never to the logged data. Per locked decision: complete shell output is always logged to Postgres.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    1. **`packages/tools/src/shell/index.ts`** (TOOL-01):
       ...
       Implementation: Use `child_process.spawn` per research Pattern 3. Pass `signal` from AbortSignal to spawn options. Set `shell: false` always (security — even in unrestricted mode, avoid shell injection from agent-generated commands). Collect stdout and stderr as strings. Return the FULL `{ stdout, stderr, exitCode }` — the shell tool itself MUST NOT truncate output. Per locked decision ("Log everything: complete shell output. Storage is cheap, missing data is not recoverable"), the tool returns all output. Truncation for LLM context protection is handled by the invocation layer (invoke.ts) AFTER the full output has been logged to Postgres.
