...
<task type="auto">
  <name>Task 2: Create @jarvis/db package with all Drizzle schemas and verify persistence</name>
  <files>
    packages/db/package.json
    packages/db/tsconfig.json
    packages/db/drizzle.config.ts
  ...
  <action>
    ...
    4. **Schema files** — follow the research code examples exactly:
       ...
       **`schema/index.ts`**: Re-export all schema objects from all schema files using `export * from './agent-state.js'` etc.

    5. **`packages/db/src/client.ts`**: Create and export `pool` (new Pool with `connectionString: process.env.DATABASE_URL!`, `max: 20`, `idleTimeoutMillis: 30_000`, `connectionTimeoutMillis: 2_000`) and `db` (drizzle(pool, { schema })). Export `type DbClient = typeof db`. Add a `shutdown()` function that calls `pool.end()` for graceful shutdown.
    ...
    7. **Verify persistence:** After pushing the schema, run a quick verification script (inline via `tsx -e`):
    ...
    Run `pnpm install` from root after creating packages/db/package.json. Then run `DATABASE_URL=postgres://jarvis:jarvis@localhost:5432/jarvis pnpm --filter @jarvis/db run db:push` to apply schema.
  </action>
  <verify>
