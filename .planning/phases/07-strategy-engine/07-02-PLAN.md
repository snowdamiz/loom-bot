...
must_haves:
  ...
  key_links:
    ...
    - from: "apps/agent/src/loop/agent-loop.ts"
      ...
      via: "evaluateOutcome called with strategyContext as 4th arg"
      pattern: "evaluateOutcome.*strategyContext"
---
...
<task type="auto">
  ...
  <action>
    ...
    **IMPORTANT preservation note:** Keep the full AVAILABLE TOOLS section (`this.registry.list()` map) at the end of systemParts, and ALL existing constraint bullet points unchanged. Only insert the strategy context block between the SUB-GOAL line and the CONSTRAINTS header. The final `content` value is `systemParts.join('\n')`.
    ...
    Also in `runGoalCycle()`, update the `evaluateOutcome()` call site to pass the strategy P&L context. The existing call at line ~316 is:
    ```typescript
    ...
    Change it to pass `this.strategyContext` as the 4th argument:
    ```typescript
    const evaluation = await this.evaluator.evaluateOutcome(
      subGoal,
      result.outcome,
    ...
    This ensures the evaluator receives the portfolio P&L context (when available) for strategy-aware evaluation. When `this.strategyContext` is `undefined` (non-strategy goals), the parameter falls through as `undefined` and the evaluator skips the P&L section — fully backwards-compatible.
...
<task type="auto">
  ...
  <action>
    ...
    **Modify `apps/dashboard/src/routes/api.ts`:**
    ...
    Also add a `GET /strategies` route that returns the strategy portfolio with P&L:
    ```typescript
    ...
    NOTE: `GET /strategies` (mounted as `GET /api/strategies`) is a data API for operator visibility (e.g., curl, scripts, monitoring), NOT a dashboard UI feature. Dashboard UI for strategy data is deferred to Phase 8 self-extension per locked decision. This endpoint supports the operator's ability to inspect the agent's strategy portfolio without a dedicated UI.
    ...
    **Modify `apps/dashboard/src/app.ts`:**
    ...
    ```typescript
    // Phase 7: Strategy engine — goal seeding + strategy portfolio API
