...
must_haves:
  truths:
    ...
    - "evaluator.ts is NOT modified — no P&L augmentation"
  artifacts:
...
The portfolio context is intentionally minimal and domain-agnostic: it lists what the agent is pursuing and in what state. The agent uses its own LLM reasoning and existing tools (db, http, browser, shell) to evaluate strategies, query P&L, research opportunities, and make all decisions. No financial logic is baked into the wiring.
...
<task type="auto">
  ...
  <action>
    ...
    **`buildPortfolioContextPrompt(strategies: Strategy[]): string`**
    ...
    Takes plain `Strategy[]` — NOT a P&L-enriched type. The prompt contains zero financial data. The agent queries financial data via its `db` tool when its LLM reasoning decides to.
    ...
    You can use the db tool to query detailed information about any strategy (costs, revenue, metadata).
    Evaluate your strategies, scale what works, kill what doesn't, and discover new approaches.
    ```

    Keep it to one line per strategy with status and hypothesis. Truncate hypothesis text to 80 chars. Show `lastTransitionReason` only for paused/killed strategies. The agent can always query the `db` tool for full details including any financial data it needs.

    **IMPORTANT: No financial data in the prompt.** No capital, no revenue, no costs, no P&L. No money-specific guidance like "scale strategies with positive P&L". The agent figures out evaluation criteria based on its goal using its own reasoning.
    ...
    **Do NOT modify `evaluator.ts`.** The evaluator does not get P&L context or any strategy-specific augmentation. The agent's LLM reasoning handles all evaluation logic using its existing tools. The evaluator continues to work exactly as it does today.

    **Do NOT:**
    - Add any financial data to the portfolio prompt
  ...
  <verify>
    ...
    `buildPortfolioContextPrompt` is exported from `strategy-prompts.ts` and takes `Strategy[]` (not StrategyWithPnl).
    ...
    Portfolio prompt contains NO financial data (no capital, revenue, costs, P&L).
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    ...
    **Create `apps/dashboard/src/routes/api.ts`:**
    ...
    Import `asc` from `@jarvis/db`. Do NOT import `getPnl` — the API returns plain strategy rows. The agent (or operator via other tools) queries P&L separately when needed.
  ...
  <verify>
    ...
    `GET /api/strategies` returns plain strategy rows (no P&L enrichment).
    No `getPnl` import in api.ts.
  </verify>
  <done>
...
7. `GET /api/strategies` returns plain strategy rows (no P&L)
8. `evaluator.ts` is NOT modified
9. No hard-coded kill triggers, P&L logic, or domain-specific logic exist anywhere
...
- Portfolio context lists status and hypothesis only — zero financial data
- Evaluator is untouched — no P&L augmentation
...
- No P&L logic, no financial logic, no domain-specific logic anywhere in Phase 7 code
</success_criteria>
