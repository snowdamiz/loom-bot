...
<task type="auto">
  ...
  <action>
    ...
    **Modify `apps/dashboard/src/routes/api.ts`:**
    ...
    NOTE: `GET /api/strategies` is a data API for operator visibility (e.g., curl, scripts, monitoring), NOT a dashboard UI feature. Dashboard UI for strategy data is deferred to Phase 8 self-extension per locked decision. This endpoint supports the operator's ability to inspect the agent's strategy portfolio without a dedicated UI.
