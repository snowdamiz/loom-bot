...
must_haves:
  ...
  key_links:
    ...
    - from: "apps/agent/src/loop/agent-loop.ts"
      to: "apps/agent/src/recovery/journal.ts"
      via: "checkpoint() called after each sub-goal completion in runGoalCycle"
      pattern: "checkpoint\\(this\\.db"
...
<task type="auto">
  <name>Task 1: Create journal checkpoint system and wire into AgentLoop</name>
  <files>
    apps/agent/src/recovery/journal.ts
    apps/agent/src/loop/agent-loop.ts
  </files>
...
**Wire checkpoint into AgentLoop.runGoalCycle** — update `apps/agent/src/loop/agent-loop.ts`:
- Import `{ checkpoint }` from `../recovery/journal.js`
- In `runGoalCycle()`, AFTER executeSubGoal() returns (and after the optional evaluator call added in Plan 02), call:
  ```
  ...
  ```
- This MUST happen before proceeding to the next sub-goal (RECOV-01 requirement). The checkpoint() function already has 3-retry-with-halt semantics, so if it fails, the agent loop stops for this goal (preventing uncheckpointed progress).
- Also call `clearJournal(this.db, goalId)` when the goal completes (all sub-goals done) to clean up.
  </action>
  <verify>
`cd /Users/sn0w/Documents/dev/jarvis/apps/agent && npx tsc --noEmit` compiles cleanly. Journal functions are importable. AgentLoop.runGoalCycle calls checkpoint() after each sub-goal completion.
  </verify>
  <done>
Journal checkpoint system writes to agent_state after each sub-goal completion with mandatory success (3 retries before halting). Reads support startup recovery with Set of completed sub-goal IDs. Cleanup available on goal completion. AgentLoop.runGoalCycle is wired to call checkpoint() after each sub-goal (RECOV-01).
  </done>
</task>
